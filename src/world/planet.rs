...
pub fn load_model(path: &str) -> (Vec<Vertex>, Vec<u32>, gltf::image::Data, gltf::image::Data, gltf::image::Data, gltf::image::Data, gltf::image::Data) {
    ...
    // Failsafe blank displacement map (since standard GLTF 2.0 hides it in extensions)
    let disp_image = gltf::image::Data { pixels: [128, 128, 128, 255].to_vec(), format: gltf::image::Format::R8G8B8A8, width: 1, height: 1 };
    ...
    for mesh in document.meshes() {
        for primitive in mesh.primitives() {
            ...
            let tex_coords: Vec<[f32; 2]> = if let Some(tc) = reader.read_tex_coords(0) { tc.into_f32().collect() } else { vec![[0.0, 0.0]; positions.len()] };
            let tangents: Vec<[f32; 4]> = if let Some(t) = reader.read_tangents() { t.collect() } else { vec![[1.0, 0.0, 0.0, 1.0]; positions.len()] };
    ...
    (vertices, indices, diffuse_image, normal_image, mr_image, ao_image, disp_image)
}